...
struct sharedData
{
    ...
    void resize(size_t rx_mtu, int upsample_koef)
    {
        real_p.resize(rx_mtu, 0);
        imag_p.resize(rx_mtu, 0);
        real_p_aft_con.resize(rx_mtu, 0);
...
void run_backend(sharedData &sh_data) {
    ...
    sh_data.resize(sdr.rx_mtu, sh_data.upsample_koef);
    CostasState costas;
    GardnerState gardner;

    fftwf_complex* in = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * sdr.rx_mtu);
    fftwf_complex* out = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * sdr.rx_mtu);
    fftwf_plan plan = fftwf_plan_dft_1d(sdr.rx_mtu, in, out, FFTW_FORWARD, FFTW_ESTIMATE);
