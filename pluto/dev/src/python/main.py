...
if answer == 2:
    nx = np.fromfile(
        "/home/plutoSDR/Документы/someotb/sdr/pluto/dev/pcm/rx.pcm",
        dtype=np.int16,
    )
else:
    nx = np.fromfile(
        "/home/plutoSDR/Документы/someotb/sdr/pluto/dev/pcm/tx.pcm",
        dtype=np.int16,
    )

rx = np.fromfile("/home/plutoSDR/Документы/someotb/sdr/pluto/dev/pcm/rx.pcm", dtype=np.int16)
tx = np.fromfile("/home/plutoSDR/Документы/someotb/sdr/pluto/dev/pcm/tx.pcm", dtype=np.int16)
